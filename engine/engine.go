...
// OpenWindow - opens a new windows and initialize openGL
func OpenWindow(onReady func(), onNewFrame func()) {
	initGlfw()
	win, err := glfw.CreateWindow(width, height, "G PA D ID", nil, nil)
	if err != nil {
		panic(err)
	}
	window = win
...
func RegisterOnKeyCallback(onKeyCallback glfw.KeyCallback) {
	if window != nil {
...
// Draw - draw a vao on the current window
